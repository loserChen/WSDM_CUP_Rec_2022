...
def lgb_ranker_model(train,test,k_fold=5,seed=2022):
    ...
    for n_fold, valid_user in enumerate(user_set):
        ...
        lgb_ranker = lgb.LGBMRanker(boosting_type='gbdt', num_leaves=7, reg_alpha=0.0, reg_lambda=1,
                                    ...
                                    subsample_freq=1,learning_rate=0.03, min_child_weight=10, random_state=seed, 
                                    n_jobs=-1)  
...
score_list_t1_1, test_t1_pred_1 = lgb_ranker_model(Train1,Test1,5,2000)
score_list_t1_2, test_t1_pred_2 = lgb_ranker_model(Train1,Test1,5,10)
print('====================================== t2 ======================================')
score_list_t2,test_t2_pred = lgb_ranker_model(Train2,Test2,5,2022)
score_list_t2_1,test_t2_pred_1 = lgb_ranker_model(Train2,Test2,5,2000)
score_list_t2_2,test_t2_pred_2 = lgb_ranker_model(Train2,Test2,5,10)

output_cols = ['userId','itemId','merge_score']
### t1
score_df_t1 = pd.concat(score_list_t1, axis=0)

score_df_t1_1 = pd.concat(score_list_t1_1, axis=0)

score_df_t1_2 = pd.concat(score_list_t1_2, axis=0)

score_df_t1 = score_df_t1.merge(score_df_t1_1,on=['userId','itemId'],how='left')
score_df_t1 = score_df_t1.merge(score_df_t1_2,on=['userId','itemId'],how='left')

score_df_t1['merge_score'] = (score_df_t1['pred_score_x']+score_df_t1['pred_score_y']+score_df_t1['pred_score'])/3

test_t1_pred = test_t1_pred.merge(test_t1_pred_1,on=['userId','itemId'],how='left')
test_t1_pred = test_t1_pred.merge(test_t1_pred_2,on=['userId','itemId'],how='left')

test_t1_pred['merge_score'] = (test_t1_pred['pred_score_x']+test_t1_pred['pred_score_y']+test_t1_pred['pred_score'])/3

score_df_t1 = score_df_t1[output_cols].rename(columns={'merge_score':'score'})
test_t1_pred = test_t1_pred[output_cols].rename(columns={'merge_score':'score'})
...
score_df_t2_1 = pd.concat(score_list_t2_1, axis=0)

score_df_t2_2 = pd.concat(score_list_t2_2, axis=0)

score_df_t2 = score_df_t2.merge(score_df_t2_1,on=['userId','itemId'],how='left')
score_df_t2 = score_df_t2.merge(score_df_t2_2,on=['userId','itemId'],how='left')

score_df_t2['merge_score'] = (score_df_t2['pred_score_x']+score_df_t2['pred_score_y']+score_df_t2['pred_score'])/3

test_t2_pred = test_t2_pred.merge(test_t2_pred_1,on=['userId','itemId'],how='left')
test_t2_pred = test_t2_pred.merge(test_t2_pred_2,on=['userId','itemId'],how='left')

test_t2_pred['merge_score'] = (test_t2_pred['pred_score_x']+test_t2_pred['pred_score_y']+test_t2_pred['pred_score'])/3


score_df_t2 = score_df_t2[output_cols].rename(columns={'merge_score':'score'})
test_t2_pred = test_t2_pred[output_cols].rename(columns={'merge_score':'score'})
...
mkdir('./merge/result_rank1/t1')
mkdir('./merge/result_rank1/t2')
...
score_list_t1_1, test_t1_pred_1 = lgb_ranker_model(Train1,Test1,10,2000)
score_list_t1_2, test_t1_pred_2 = lgb_ranker_model(Train1,Test1,10,10)
print('====================================== t2 ======================================')
score_list_t2,test_t2_pred = lgb_ranker_model(Train2,Test2,10,2021)
score_list_t2_1,test_t2_pred_1 = lgb_ranker_model(Train2,Test2,10,2000)
score_list_t2_2,test_t2_pred_2 = lgb_ranker_model(Train2,Test2,10,10)
...
mkdir('./merge/result_rank2/t1')
mkdir('./merge/result_rank2/t2')
score_df_t1.to_csv(output_dir+'t1/valid_pred.tsv',index=False,sep='\t')
test_t1_pred.to_csv(output_dir+'t1/test_pred.tsv',index=False,sep='\t')
score_df_t2.to_csv(output_dir+'t2/valid_pred.tsv',index=False,sep='\t')
test_t2_pred.to_csv(output_dir+'t2/test_pred.tsv',index=False,sep='\t')